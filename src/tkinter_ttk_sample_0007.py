...
def show_original_temp_path(current_image: dict) -> None:
    original_temp_path = current_image["original_temp_path"]
    if original_temp_path is None:
        messagebox.showinfo("原寸tempパス", "原寸tempファイルはありません。")
        return

    messagebox.showinfo("原寸tempパス", original_temp_path)
...
def main() -> None:
    ...
    root.title("tkinter / ttk Button Sample 0007")
    ...
    tk_button = tk.Button(
        frame,
        text="tkinter.Button",
        command=lambda: show_original_temp_path(current_image),
    )
